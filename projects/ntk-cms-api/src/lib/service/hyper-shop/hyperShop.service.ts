...
export class HyperShopService extends ApiServerBase {
  ...
  ServiceUniversalAction(
    model: UniversalActionModel
  ): Observable<ErrorExceptionResult<UniversalActionModel>> {
    if (model == null) {
      model = new UniversalActionModel();
    }
    return this.http
      .post(
        this.getBaseUrl() + this.getModuleControllerUrl() + '/UniversalAction',
        model,
        {
  ...
  ServiceCheckStatus(): Observable<
    ErrorExceptionResult<CheckStatusActionModel>
  > {
    return this.http
