...
@Injectable()
export class FileCategoryService extends ApiCmsServerBase<
  FileCategoryModel,
  number,
  FilterModel
> {
  ...
  ServiceGetAllInCategoryById(
    ParentId: number
  ): Observable<ErrorExceptionResult<FileCategoryModel>> {
    return this.http
      .get(
        this.getBaseUrl() +
  ...
  ServiceCreateCompressDownload(
    model: FileCompressDtoModel
  ): Observable<ErrorExceptionResultBase> {
    return this.http
      .post(
        this.getBaseUrl() +
          this.getModuleControllerUrl() +
          '/CreateCompressDownload/',
        model,
        {
          headers: this.getHeaders(),
        }
